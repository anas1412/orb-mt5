...
ax2=ax.twinx()
for rows,lab,col in ((A,"close-pos 0.25",ACC),(B,"close-pos 0.50",POS)):
    pct=[0.0]; cum=0.0
...
ax.set_title("Cumulative return — gold Asia 2026, Mon–Thu, fixed 2% risk, no compounding",
...
R=[r['R'] for r in A]
ax.hist(R,bins=24,color=ACC,alpha=.75,edgecolor="white",lw=.6)
ax.axvline(0,color=MUT,lw=1)
ax.axvline(statistics.mean(R),color=NEG,lw=1.6,label="mean %+.2f R"%statistics.mean(R))
ax.set_title("Outcome distribution, cp 0.25",fontsize=11,color=INK,weight="bold",loc="left")
...
for i,(rows,lab,col) in enumerate(((A,"cp 0.25",ACC),(B,"cp 0.50",POS))):
    vals=[statistics.mean([r['R'] for r in rows if r['t'].month==m]) for m in mo]
    ax.bar([m+(i-.5)*w for m in mo],vals,width=w,color=col,alpha=.85,label=lab)
...
ax.set_ylabel("R per trade",fontsize=9,color=MUT); ax.legend(frameon=False,fontsize=9)
for s in ("top","right"): ax.spines[s].set_visible(False)
ax.grid(axis="y",color=LINE,lw=.6)
...
fig.savefig("edge_2026.png",bbox_inches="tight",facecolor="white")
print("saved. worst drawdown %.1f%%, longest losing run %d (= %.0f%%)"%(abs(min(dd)),max(seq),max(seq)*RISK))
