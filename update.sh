...
sed -i -e "s|^FromDate=.*|FromDate=$BAR_FROM|" -e "s|^ToDate=.*|ToDate=$TO|" mql5/config/dump.ini
...
git add README.md orb.html index.html orbnq.html mql5/config/tester.ini mql5/config/dump.ini trades \
...
if [ -n "$(git status --short | grep -v '^[MADR] ')" ]; then
  echo "  left uncommitted (not generated by this run):"
  git status --short | grep -v '^[MADR] ' | sed 's/^/    /'
fi
